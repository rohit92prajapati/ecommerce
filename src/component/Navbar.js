...
import { filtersearchproduct, filterlogin } from "../redux/counterSlice";
...
export default function Navbar() {
  ...
  const addProducts = useSelector((state) => state.filterProduct.addProducts);
  const top100Films = [
            ...
            <Search>
              <Autocomplete
                ...
                onInputChange={(event, inputValue) => {
                  ...
                  const filterData = addProducts.filter(
                    (el) => el.title === inputValue
                  );
                  dispatch(filtersearchproduct(filterData));
                }}
                disableClearable
                options={addProducts.map((option) => option.title)}
                renderInput={(params) => (
                  <TextField
                    {...params}
